...
import { enrichTrustpilotDomainFallback } from "@/lib/enrichment/dataforseo";
...
export async function POST(request: NextRequest) {
  ...
  const [job] = await db
    ...
    .values({ type: "trustpilot_domain", status: "pending", totalItems: 0, processedItems: 0, errorCount: 0 })
    .returning();

  enrichTrustpilotDomainFallback(job.id, priority as 1 | 2).catch(async (err) => {
