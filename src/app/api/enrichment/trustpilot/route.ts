...
import { enrichTrustpilot } from "@/lib/enrichment/dataforseo";
...
export async function POST(request: NextRequest) {
  ...
  const [job] = await db
    ...
    .values({ type: "trustpilot", status: "pending", totalItems: 0, processedItems: 0, errorCount: 0 })
    .returning();

  enrichTrustpilot(job.id, undefined, priority as 1 | 2).catch(async (err) => {
