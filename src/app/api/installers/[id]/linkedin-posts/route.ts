  ...
  if (apiKey && newSignals > 0) {
    try {
      ...
      if (unscoredFiltered.length > 0) {
        ...
        const result = await model.generateContent(`You are scoring LinkedIn posts from employees of "${installer?.companyName || "an installer company"}". The user sells marketing, lead generation, and software services to these companies.
